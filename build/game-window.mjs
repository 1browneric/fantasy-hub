...
// Christmas - without a hand-kept calendar. Prints a word, then the minutes to
// the next kickoff (-1 when there is none on the slate):
...
if (process.env.FORCE_WINDOW) { console.log(process.env.FORCE_WINDOW + ' -1'); process.exit(0); }
let d;
try {
  const r = await fetch('https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard');
  if (!r.ok) throw new Error('HTTP ' + r.status);
  d = await r.json();
} catch (e) { console.error('schedule unreadable: ' + e.message); console.log('unknown -1'); process.exit(0); }
const now = Date.now();
...
const m = Number.isFinite(mins) ? Math.max(0, Math.round(mins)) : -1;
if (live) { console.error(`${live} game(s) live`); console.log('live ' + m); }
else if (mins <= LEAD) { console.error(`next kickoff in ${m} min`); console.log('soon ' + m); }
else { console.error(Number.isFinite(mins) ? `next kickoff in ${(mins / 60).toFixed(1)} h` : 'nothing left on the slate'); console.log('off ' + m); }
